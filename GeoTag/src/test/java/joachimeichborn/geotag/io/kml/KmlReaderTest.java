...
public class KmlReaderTest {
	...
	public void testReading(final String aKmlFilename, final List<PositionData> aExpectedPositions) throws IOException {
		...
		FileUtils.copyURLToFile(KmlReader.class.getResource(aKmlFilename), kmlFile);

		final KmlReader reader = new KmlReader(kmlFile.toPath());

		final Track track = reader.read();
